...
async function render_graph(graph_nodes, graph_links, target) {
	...
	app.simulation = d3.forceSimulation(app.nodes)
		...
		.force("center", d3.forceCenter(app.viewport_width/3, app.viewport_height/3))
		.on('tick', ticked);
...
// ################ EVENT LISTENERS D3 DO NOT WORK WELL WITH VUE ####################
function setD3EventListeners(){
...
// TODO Replace
	...
	d3.select("#sticky").on("change", function() {
		...
		var sticky = app.sticky_mode;
		console.log("d3 el sticky change triggered", sticky)
	...
	d3.select("#apply_settings_button").on("click", function() {
		console.log("++++++++++++in graph apply settings button +++++++++++++++++++++")
		for (let i = 0; i < app.clusters.length; i++) {
			...
			console.log("d3 apply settings cluster loop", i, "with cluster", cluster_name)
			...
			values = cluster_node_exists(cluster_id)
			exists = values[0]
			currentname = values[1]

			console.log("d3 cluster_node CRUD decision values", exists, add_cluster_node, currentname)
